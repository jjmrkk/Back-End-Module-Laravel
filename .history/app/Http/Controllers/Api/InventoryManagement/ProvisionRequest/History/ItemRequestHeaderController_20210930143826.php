...
namespace App\Http\Controllers\Api\InventoryManagement\ProvisionRequest\History;
...
class ItemRequestHeaderController extends Controller
{
    ...
    public function itemRequestCount()
    {
        try {
            ...
            $itemRequestHeaderCount = DB::table('item_request_headers as header')
                                ->where('header.status_id', 13)
                                ->join('item_request_header_logs as log', function ($join) {
                                    $join->on('header.id', '=', 'log.item_request_header_id')
                                        ->whereColumn('header.updated_at', 'log.created_at')
