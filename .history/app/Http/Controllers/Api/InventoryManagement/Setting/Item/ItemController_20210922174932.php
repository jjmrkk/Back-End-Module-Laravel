...
class ItemController extends Controller
{
    ...
    public function store(Request $request)
    {
        try {
            ...
            $contains_empty = count($request->item_attributes_list);
            $contains_empty1 = count(array_filter($request->item_attributes_list, "strlen"));

            $contains_empty = in_array("", $request->item_attributes_list, true);

            return response()->json(['message' => $contains_empty1 .= '=' . $contains_empty], $this->errorStatus);
            ...
            if ($attribute['attribute_value'] != array_filter( $attribute['attribute_value'] )) 
            return response()->json(['message' => "Empty: $attrvalue"], $this->errorStatus);
            else
            return response()->json(['message' => "Not Empty: $attrvalue"], $this->errorStatus);
