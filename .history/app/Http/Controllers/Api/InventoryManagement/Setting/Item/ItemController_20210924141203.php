...
class ItemController extends Controller
{
    ...
    public function store(Request $request)
    {
        try {
           ...
           $item_code = stringValue($request->category_id.= "-" .$request->brand_id.= "" .$request->model_id.= "-" .$request->warehouse_id.= "-" .$str);
          // return response()->json(['message' => "$item_code"], $this->errorStatus);

            $itemname_attr = ucwords($item_name.= " " .$attrname);

            if ($fieldCount != $filledCount) {
                return response()->json(['message' => "Please Fill Out All Attributes Fields $itemname_attr"], $this->errorStatus);
            }
            ...
            $item = DB::table('item_details')->insert([
                ...
                'minimum' => $request->minimum,          // Something is wrong on angular validation
