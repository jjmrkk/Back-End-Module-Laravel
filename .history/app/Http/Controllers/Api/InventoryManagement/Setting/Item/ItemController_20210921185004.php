    ...
    public function store(Request $request)
    {
        try {
            ...
            foreach ($request->item_attributes_list as $attribute) {
                $attrname .= " " . $attribute['attribute_name']['attribute_value'];
                $attrvalue .= ", " . $attribute['attribute_value'];
            }

        $attrdetails = json_decode($request->item_attributes_list['attribute_name']);

        return response()->json(['message' => "test1: $attrname"], $this->errorStatus);



            $name_check = DB::table('item_details')->where('name', ($name . ", " . $attrname))->exists();
            if ($name_check) {
                return response()->json(['message' => "Duplicate Name Entry Found for $name$attrname."], $this->errorStatus);
            }
