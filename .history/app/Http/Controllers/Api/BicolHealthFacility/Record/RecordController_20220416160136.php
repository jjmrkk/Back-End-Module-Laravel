...
class RecordController extends Controller
{
    ...
    { 
     ...
     try {
       ...
       //     ->where('philhealth_id', $request->philhealth_id)
        //    ->exists();
        //    if ($phil_health_no) {
        //    return response()->json(['message' => "Duplicate PhilHealth No. Entry Found for $request->philhealth_id. Please Contact your system administrator "], $this->errorStatus);
        //    }
        ...
        if ($request->action == 'pif')
        {
            ...
            foreach($previous_illnesses as $a)
            {
                ...
                if($request->has('id:'.$a->id) && $request['id:'.$a->id] == false)
                {
                    $previous_illnesses_checked[]; //null
                }
            }
        ...
        if ($request->action == 'fhff')
        {
            ...
            foreach($family_history_father as $a)
            {
               ...
               {
                   $family_history_father_checked = []; //null
               }
            }
        ...
        if ($request->action == 'fhmf')
        {
            ...
            foreach($family_history_mother as $a)
            {
               ...
               {
                   $family_history_mother_checked = []; //null
               }
            }
        ...
        if ($request->action == 'pi')
        {
            ...
            foreach($present_illnesses as $a)
            {
               ...
               {
                   $present_illnesses_checked = []; //null
               }
            }
        ...
        if ($request->action == 'ih')
        {
            ...
            foreach($immunization_history as $a)
            {
               ...
               {
                   $immunization_history_checked = []; // null
               }
            }
    ...
    public function filter($date_from, $date_to, $checkbox1, $checkbox2)
    { 
        try {     
