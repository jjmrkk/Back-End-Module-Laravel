...
public function test ()
{
    ...
    return Carbon::parse($info->date_of_birth)->age;
    ->first();
    ...
    public function pdf(Request $request)
    {
        try { 
            ...
            if($request->action == 'EKAS')
            {
                ...
                $view = \View::make('bicol_health_facility/forms/EKAS_Form', ['info'=>$info]);
                $html_content = $view->render();
                $pdf = new TCPDF('P', PDF_UNIT, 'A4', true, 'UTF-8', false);
                // set document information
                $pdf->SetCreator(PDF_CREATOR);
            ...
            if($request->action == 'EPRESS')
            {
                ...
                $view = \View::make('bicol_health_facility/forms/EPRESS_Form', ['info'=>$info]);
                $html_content = $view->render();
                $pdf = new TCPDF('P', PDF_UNIT, 'A4', true, 'UTF-8', false);
                // set document information
                $pdf->SetCreator(PDF_CREATOR);
...
//------------------------------------------------------------------------------------------//
                ...
                $view = \View::make('bicol_health_facility/forms/FPEF_Form', ['info'=>$complete_info]);
                $html_content = $view->render();
                $pdf = new TCPDF('P', PDF_UNIT, 'A4', true, 'UTF-8', false);
                // set document information
                $pdf->SetCreator(PDF_CREATOR);
