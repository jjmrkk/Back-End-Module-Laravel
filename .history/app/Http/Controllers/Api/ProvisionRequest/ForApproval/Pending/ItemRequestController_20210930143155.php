...
namespace App\Http\Controllers\Api\ProvisionRequest\ForApproval\Pending;
                ...
                $itemRequestHeaders = DB::table('item_request_headers as header')
                                            ->where(function ($query) use ($firstApprovals, $secordApprovals) {
                                                if($firstApprovals)
                                                {
                                                    ...
                                                    {
                                                        $query->orWhere('header.status_id', 3)
                                                ...
                                                if($secordApprovals)
                                                {
                                                    ...
                                                    {
                                                        $query->orWhere('header.status_id', 6)
    ...
    public function itemRequestCount()
    {
        try {
            ...
            if($firstApprovals)
            {
                foreach($firstApprovals as $firstApproval)
                {
                    $count = DB::table('item_request_headers')
                                ->where('status_id', 3)
                                ->where('description->requestor->business_unit_id', $firstApproval->business_unit_id)
                                ->where('description->requestor->group_id', $firstApproval->group_id)
                                ->count();
            ...
            if($secordApprovals)
            {
                foreach($secordApprovals as $secordApproval)
                {
                    $count = DB::table('item_request_headers')
                                ->where('status_id', 6)
                                ->where('description->requestor->business_unit_id', $secordApproval->business_unit_id)
                                ->where('description->requestor->group_id', $secordApproval->group_id)
                                ->count();
                    $itemRequestHeaderCount += $count;
                }
            }
