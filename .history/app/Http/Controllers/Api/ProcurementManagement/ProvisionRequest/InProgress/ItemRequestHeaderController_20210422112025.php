...
namespace App\Http\Controllers\Api\ProcurementManagement\ProvisionRequest\InProgress;
...
class ItemRequestHeaderController extends Controller
{
    ...
    {
        $userAccess = DB::table('user_access')->where('user_id', Auth::id())->select('description->procurement_management->provision_request_module as provision_request_module')->first();
    ...
    public function itemRequestCount()
    {
        try {
            ...
            $itemRequestHeaderCount = DB::table('item_request_headers')->whereIn('status_id', $this->status())->count();
