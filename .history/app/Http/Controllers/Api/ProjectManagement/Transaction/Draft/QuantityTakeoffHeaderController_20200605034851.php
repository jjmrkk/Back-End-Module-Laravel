...
namespace App\Http\Controllers\Api\ProjectManagement\Transaction\Draft;
...
class QuantityTakeoffHeaderController extends Controller
{
    ...
    public function index()
    {
        try {
            $qtoHeaders = DB::table('quantity_takeoff_headers as header')
                            ...
                            ->select(
                                'header.id', 'header.updated_at', 'header.project_id', 'header.status_id',
    ...
    {
        try {
            $qtoHeaderCount = DB::table('quantity_takeoff_headers')->where('status_id', 1)->count();
